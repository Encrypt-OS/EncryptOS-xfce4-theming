...
echo "******* Installing EncryptOS Theming for XFCE4*******" && sleep 5

echo "******* cloning dotfiles for EncryptOS - XFCE4 Theming *******" && sleep 1
    git clone https://github.com/Encrypt-OS/encryptos-xfce4-theming.git
...
echo "******* setting up theme for Light-DM: *******" && sleep 1
    wget -q --timeout=10 https://raw.githubusercontent.com/Encrypt-OS/EncryptOS-ISO/main/airootfs/etc/lightdm/slick-greeter.conf
...
echo "******* set lightdm.conf to logind-check-graphical=true ... enable slick greeter to be used *******" && sleep 1
    cp /etc/lightdm/lightdm.conf ~/encryptos-xfce4-theming/
...
echo "******* setting up xfce4 theme and settings: *******" && sleep 1
    ...
    cd ..
    rm -rf encryptos-xfce4-theming 

echo "******* All Done --- restarting System NOW! *******" && sleep 10
echo "******* Please login again and enjoy EncryptOS Theming! *******" && sleep 1

    yad --title="Restarting System" \
        --text="All done --- please login again and enjoy XFCE4 with EncryptOS Theming!" \
